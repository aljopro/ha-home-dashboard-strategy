...
describe('areaCards', () => {
    ...
    describe('buildAreaCardsSection', () => {
        it('returns heading followed by area cards', () => {
            ...
            const result = buildAreaCardsSection(areas, '');
            ...
            // Areas are sorted alphabetically, so Bedroom comes before Living Room
            expect(result[1]).toMatchObject({ type: 'area', area: 'bedroom' });
            expect(result[2]).toMatchObject({ type: 'area', area: 'living_room' });
        });
